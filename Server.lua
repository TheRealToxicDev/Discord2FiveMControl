--[[
* Editing this file could result in the script breaking
* Please contact me if assistance is needed here.
]]--
...
function GetRealPlayerName(playerId)
    if Config.ESX then|
        ...
        return "[Error] ESX IS NOT ENABLED, PLEASE CHECK THE CONFIG"
    end
end
    ...
    if string.starts(command, Config.Prefix) then
        ...
        elseif string.starts(command, Config.Prefix .. "kick") then
            ...
            if t[2] ~= nil and GetPlayerName(t[2]) ~= nil then
                sendToDiscord("Kicked Succesfully", "Succesfully kicked: " .. GetPlayerName(t[2]),16711680)
                DropPlayer(t[2],"KICKED FROM DISCORD STAFF")

            else

                sendToDiscord("Could not find", "Could not find a user with that ID. Make sure to input a Valid ID",16711680)
...
Citizen.CreateThread(function()
    ...
    while true do   
        ...
        local chan = DiscordRequest("GET","channels/" .. Config.ChannelID, {})
        if chan.data then
            local data = json.decode(chan.data)
        ...
        Citizen.Wait(Config.WaitEverTick)
    end
end)
    ...
    local connect = {
        {
            ...
            ["description"] = message,
            ["footer"] = {
                ["text"] = "Developed by ☣ Tσxιƈ Dҽʋ ☣#7308"
            },
        }
    }
